...
# PX[color]="off"                 # color is currently 'on' or 'off'
PX[log]="true"
PX[git-project-name]=""         # name of current git project or ""
# 
PX[ps1-color]=""                # command line prompts
PX[ps1-mono]=""
PX[ps1-git-color]=""
PX[ps1-git-mono]=""
# 
...
[ "$1" != "BASHRC" ] && \
    ...
    function setup_profile() {
        ...
        for cmd in git realpath cygpath code powershell; do
            ...
            */git)          has_git="true"; path_ext+=":"$(/usr/bin/dirname "$p") ;;
        ...
        if [[ "$OS" && "$OS" =~ Windows ]]; then
            if [ "${PX[clean-envar]}" = "true" ]; then
                ...
                for v in $(sed -e 's/=.*//' <<< $(env)); do
                    ...
                    [[ "$v" =~ ^(RPATH|RPWD|PX_color|START_DIR|PATH|USER|HOSTNAME|HOME|LANG|SHELL|PWD|TERM|OSTYPE|USERPROFILE|SYSTEMROOT|PROFILEREAD)$ ]] && continue
                    remove+=($v)
                done
            ...
            # export HOME="$(/usr/bin/realpath $HOME)"
            # 
        ...
        # [ -f "${HOME}/.profile-common" ] && \
        #     builtin source "${HOME}/.profile-common" || \
        #     echo "could not find: \$HOME/.profile-common"
        # 
        case "$SHELL" in
        *bash)
            # [ "${PX[log]}" ] && echo -n ".profile" && local bashrclog=" -> .bashrc"
...
# set 'has_git' and 'has_realpath' if not coming through .login shell
[ -z "$has_git" -a -z "$has_realpath" ] && \
...
# if [[ ! "$SHELL" =~ zsh ]]; then
    [ "$PX_color" = "on" ] && color on || color off
# fi
